...
linha_contagem_y = 720
ids_contados = set()
contador_total = 0
veiculos_ids = [2, 3, 5, 7]  # carro, moto, ônibus, caminhão
...
# Gerar relatório final em arquivo .txt
caminho_relatorio = os.path.join(pasta_saida, "relatorio.txt")

with open(caminho_relatorio, "w") as f:
    f.write("RELATORIO DE CONTAGEM DE VEiCULOS\n")
    f.write("=" * 40 + "\n")
    f.write(f"Total de veiculos contados: {contador_total}\n")
    f.write(f"IDs unicos contados ({len(ids_contados)}):\n")
    f.write(", ".join(map(str, sorted(ids_contados))) + "\n")
    f.write("\nLinha de contagem Y: " + str(linha_contagem_y))
    f.write("\nModelo usado: yolov8m.pt + BoT-SORT")
